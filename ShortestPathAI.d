...
public class ShortestPathAi
{
  private:
    ...
    WTreeNode!TurnInstance destination;
    ...
    void gameLoop()
    {
      ...
      if (winner == p_num)
      {
        writefln("This Shortest Path AI wins!");
      }
      else
      {
        writefln("This Shortest Path AI lost!");
      }
    }
    ...
    {
      Command ret;
      if (destination !is null && getCommandFromTree(ret, currentTurn, destination))
      {
        writefln("%s", ret);
        return ret;
      }
      // If that doesn't work, then recreate the tree
      destination = getShortestPath(currentTurn, 12, p_num);
      destination.printPathFromParent();
      getCommandFromTree(ret, currentTurn, destination);
      writefln("%s", ret);
      return ret;
    }

    bool getCommandFromTree(out Command output, TurnInstance currentTurn, WTreeNode!TurnInstance destination)
    {
      // First check if we are on the correct path
      auto traversal = destination;
      destination.printPathFromParent();
      writeln();
      while (traversal.parent !is null
          && traversal.parent.contents != currentTurn)
      {
        traversal = traversal.parent;
      }
      if (traversal !is null && traversal.parent !is null
          && traversal.parent.contents == currentTurn)
      {
        output = findDifference(currentTurn, traversal.contents);
        return true;
      }
      return false;
    }
...
private Command getStep(uint p_num, Hand ours, Hand theirFirst, Hand theirSecond)
{
  // What to return
  Command ret;
